...
Why? Writing in a point-free style in Elisp usually only needs a few basic
helpers. Many similar libraries are much larger than that, while =fp= tries to
keep the common pieces compact.
...
Run the complete local gate (ERT tests, Checkdoc, and byte compilation with
